...
from app.utils.exceptions import PasswordException, ActiveException, UserExistException, EmailExistException, ParameterException
...
class User(Base):
    ...
    referrer = Column(String(32), nullable=True, comment="推荐人")
    ...
    def generate_invite_code():
        return random.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', k=8)
    ...
    def create(body: RegisterBody):
        ...
        invite_code = body.inviteCode or getattr(body, 'invite_code', None)
        if invite_code:
            invite_result = db.session.execute(select(Invite).where(Invite.code == invite_code))
            invite = invite_result.scalar_one_or_none()
            user_invite_result = db.session.execute(select(User).where(User.invite_code == invite_code))
            invite_owner = user_invite_result.scalar_one_or_none()
            
            if not invite and not invite_owner:
                raise ParameterException(message="无效的邀请码")
            
            if invite_owner:
                user.referrer = invite_owner.username
            else:
                user.referrer = "seed"
        ...
        while db.session.execute(select(User).where(User.invite_code == user.invite_code)).scalar():
            user.invite_code = User.generate_invite_code()
    ...
    def verify_register(cls, body: RegisterBody):
        ...
        if body.email and db.session.execute(select(cls).where(cls.email == body.email)).scalar():
