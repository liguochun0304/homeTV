...
from app.form.admin import PermissionsResponse, UsersQuery, GetUsersResponse, ModifyPasswordBody, CreateRoleBody, \
    RolesQuery, GetRolesResponse, UpdateRoleBody, UserRoleBody, RolePermissionBody, SitesResponse, UpdateSitesBody
...
@api.get("/sites", responses={"200": SitesResponse})
@login_required
def get_sites():
    """获取所有站点"""
    sites = db.session.execute(select(Site).order_by(Site.id)).scalars().all()
    data = [site.data() for site in sites]
    return response(data=data)


@api.post("/sites")
@login_required
def update_sites(body: UpdateSitesBody):
    """更新站点列表"""
    try:
        db.session.execute(select(Site)).delete()
        for s in body.sites:
            site = Site()
            site.key = s.key
            site.name = s.name
            site.api = s.api
            site.active = s.active
            db.session.add(site)
        db.session.commit()
